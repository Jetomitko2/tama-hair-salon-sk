  ...
  return <div className="relative h-screen overflow-hidden">
      ...
      <div className="absolute inset-0 flex items-center justify-center text-center">
        <div className="max-w-4xl px-4 sm:px-6 text-white animate-fade-up">
          ...
          <p className="text-base sm:text-lg md:text-xl max-w-2xl mx-auto leading-relaxed drop-shadow-md mb-6 sm:mb-8 px-2">Moje meno je Tamara. Pracujem ako kaderníčka už viac ako 20 rokov. Mám bohaté skúsenosti v starostlivosti o vlasy a tvorbe krásnych účesov pre každú príležitosť.</p>
      ...
      <div className="absolute bottom-8 left-1/2 transform -translate-x-1/2 flex space-x-2">
        {images.map((_, index) => <button key={index} onClick={() => setCurrentImage(index)} className={`w-3 h-3 rounded-full transition-smooth ${index === currentImage ? "bg-white" : "bg-white/50 hover:bg-white/75"}`} />)}
      </div>
    </div>;
};
